...
subclasses, `Dog` and `Cat`, that **inherit** from it — then write a couple of
functions that **create instances** of them.
...
- `adopt(name, species)` that **creates and returns** the matching instance
  (a `Dog`, a `Cat`, or a plain `Animal`).
- `make_animals(names)` that builds a **list of `Animal` instances**.

## Key ideas

- A **class** is a blueprint; an **object** (instance) is one thing built from it.
- You **create an object** by calling the class like a function: `Dog("Rex")`
  makes a new `Dog` you can store, use, or return.
...
Pass all tests in the **Testing** panel, then commit,
