...
- `linear_search_complexity()` — the **analysis**: name the search's worst-case
  Big O.
...
- Read the operation format carefully: `("enqueue", value)` has two parts;
  `("dequeue",)` has one.
- For the **analysis**, picture the worst case for a linear scan — the target
  missing or last — and how the work grows with the list size.
