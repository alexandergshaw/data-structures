...
For this onboarding assignment you only need to do one tiny thing:
...
> ✅ **Checkpoint:** `hello_course` returns the exact greeting and your edits are
> saved.
...
> ✅ **Checkpoint:** The PR shows a purple **"Merged"** label. Vercel now rebuilds
> your live site — running your tests as part of the build — so within a minute
> or two your **Dashboard** card for Assignment 0 unlocks with a **"Completed"**
> badge! 🎉
...
You don't flip any switch — **passing tests are the only thing that unlocks a
card.** When you merge, Vercel rebuilds your site, and during that build it runs
each assignment's tests and records the result in `lib/test-status.json`. When
*all* of an assignment's tests pass, the matching card on the **Dashboard**
unlocks, shows a green **Completed** badge, and reveals that topic's
visualization.

So the recipe is simply: make every test green in the **Testing** panel, then
commit, push, and merge. There is nothing else to remember.
